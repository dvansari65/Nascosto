  ...
  const mutation = useMutation({
    ...
    mutationFn: async (cardForm: CardForm): Promise<MintResult> => {
      ...
      try {
        ...
        if (!signer) {
          toast.error("Wallet signer unavailable. Try reconnecting.", {
            id: toastId,
          });
          return { success: false, imageRolledBack: false, tokenId: null };
        }
        ...
        const tokenId = await ShadowCardService.mintCard(
          signer,
          address,
        ...
        return {
          success: false,
          imageRolledBack: Boolean(imageCid),
          tokenId: null,
        };
      }
    },
