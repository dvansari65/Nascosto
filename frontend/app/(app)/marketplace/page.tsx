...
import { submitOffer, SubmitOfferInputs } from "@/api/submitEncryptOffer";
import { deList } from "@/api/deList";
import { DotsLoader } from "@/components/loaders/dots-loader";

export default function MarketplacePage() {
  const [activeOfferTokenId, setActiveOfferTokenId] = useState<{
    tokenId: bigint | null;
    seller: string | null;
  }>({
    tokenId: null,
  ...
  const {
    mutate,
    isPending: isSubmittingOffer,
    error: submitOfferError,
  } = submitOffer();
  ...
  const submitEncryptedOffer = async (amount: string) => {
    const payload: SubmitOfferInputs = {
      amount,
      activeOfferTokenId: activeOfferTokenId.tokenId,
      sellerKey: activeOfferTokenId.seller || "",
    };
    mutate(payload, {
      onSuccess: () => {
        toast.success("Offer submitted securely!");
        setActiveOfferTokenId({ seller: null, tokenId: null });
      },
      onError: (e) => {
    ...
    <div className="mx-auto max-w-350 px-6 lg:px-10 py-12">
      {isPending ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {cards.map(
            ({ listing, metadata, metadataLoading, metadataError }) => (
              <TradingCardItem
      ...
      {(activeOfferTokenId.tokenId && activeOfferTokenId.seller) !== null && (
        <OfferModal
          tokenId={activeOfferTokenId.tokenId?.toString() || ""}
          onClose={() => setActiveOfferTokenId({ tokenId: null, seller: null })}
